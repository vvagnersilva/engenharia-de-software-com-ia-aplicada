 ...
 *
 * O cliente do LLM é mockado (StructuredChatClient) para não depender de uma
...
class ChatFlowIntegrationTest {
    ...
    void secondChatCallRemembersPreferencesExtractedInTheFirstCall() {
        ...
        when(structuredChatClient.generateStructured(anyString(), anyString(), eq(LlmChatResponse.class)))
                .thenReturn(firstTurnResponse, secondTurnResponse);

        when(structuredChatClient.generateStructured(anyString(), anyString(), eq(ConversationSummaryData.class)))
        ...
        verify(structuredChatClient, times(2)).generateStructured(
