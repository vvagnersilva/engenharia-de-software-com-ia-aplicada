...
export const getSystemPrompt = (professionals: any[]) => {
  return JSON.stringify({
    ...
    extraction_instructions: {
      ...
      datetime: 'Parse relative dates (today, tomorrow) and times as local time in the given timezone (America/Sao_Paulo, UTC-03:00), using current_date (in UTC) as reference for "today"/"tomorrow". Then convert that local time to UTC by ADDING 3 hours, and output it as an ISO 8601 string with milliseconds and a "Z" suffix (e.g. 14:00 local time becomes "...T17:00:00.000Z"). Always apply this same +3h conversion so the same phrase always produces the same UTC timestamp.',
    ...
    examples: [
      {
        ...
        output: { intent: 'schedule', professionalId: 1, professionalName: 'Dr. Alicio da Silva', datetime: '2026-02-12T19:00:00.000Z', reason: 'check-up' },
        note: '4pm (16:00) local time in America/Sao_Paulo (UTC-03:00) -> add 3h -> 19:00 UTC'
      },
      {
        input: 'Cancel my appointment with Dr. Ana Pereira today at 11am',
        output: { intent: 'cancel', professionalId: 2, professionalName: 'Dr. Ana Pereira', datetime: '2026-02-11T14:00:00.000Z' },
        note: '11am (11:00) local time in America/Sao_Paulo (UTC-03:00) -> add 3h -> 14:00 UTC'
      },
      {
