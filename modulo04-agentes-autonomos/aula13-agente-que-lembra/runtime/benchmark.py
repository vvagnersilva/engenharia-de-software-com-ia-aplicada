...
def _diretorios_memoria_do_agente(caminho_agente: Path) -> list:
    """Extrai diretorios de memoria declarados no contrato do agente.

    Os caminhos no contrato sao relativos ao cwd (memory_store/longa/, etc.).
    """
    try:
        contratos = carregar_contratos(caminho_agente)
        tipos = contratos.get("memoria", {}).get("tipos_memoria", {})
    except Exception:
        tipos = {}

    diretorios = []
    for nome_tipo, config in tipos.items():
        if not isinstance(config, dict):
            continue
        if not config.get("ativo"):
            continue
        caminho = config.get("diretorio") or f"memory_store/{nome_tipo}/"
        diretorios.append(Path(caminho))
    return diretorios


def _marker_benchmark(caminho_agente: Path) -> Path:
    """Caminho do marker que indica 'benchmark ja rodou contra este agente'."""
    raiz_memoria = Path("memory_store")
    return raiz_memoria / f".benchmark_marker_{caminho_agente.name}"


def _limpar_memoria_do_agente(caminho_agente: Path) -> int:
    """Apaga arquivos .yaml dos diretorios de memoria do agente. Retorna qtd removida."""
    removidos = 0
    for diretorio in _diretorios_memoria_do_agente(caminho_agente):
        if not diretorio.exists():
            continue
        for arquivo in diretorio.glob("*.yaml"):
            try:
                arquivo.unlink()
                removidos += 1
            except OSError:
...
def _escrever_marker_benchmark(caminho_agente: Path, arquitetura: str) -> None:
    """Marca que o benchmark rodou para este agente."""
    marker = _marker_benchmark(caminho_agente)
...
def rodar_benchmark(caminho_agente: str, caminho_suite: str, arquitetura: str = None) -> dict:
    ...
    # Se o benchmark ja rodou antes contra este agente, memory_store esta
    # contaminada por execucoes anteriores e metricas nao serao estaveis.
    # Limpa automaticamente para garantir baseline frio.
    marker = _marker_benchmark(caminho_agente)
    if marker.exists():
        removidos = _limpar_memoria_do_agente(caminho_agente)
        print(f"  [benchmark] benchmark anterior detectado — memory_store limpo ({removidos} arquivos removidos)\n")
    ...
    _escrever_marker_benchmark(caminho_agente, nome_arquitetura)
