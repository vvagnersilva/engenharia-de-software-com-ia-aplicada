...
def _recuperar_contexto(entrada: str, memory_adapter, config_memoria: dict) -> dict:
    ...
    Se a variavel de ambiente MEMORY_DISABLED=1, retorna contexto vazio
    (usado pelo memory-eval para comparar execucao com vs sem memoria).
    """
    if os.environ.get("MEMORY_DISABLED") == "1":
        return {}
...
def _persistir_memoria(estado: dict, memory_adapter, config_memoria: dict, contratos: dict):
    ...
    Se MEMORY_DISABLED=1, e no-op (modo baseline do memory-eval).
    """
    if os.environ.get("MEMORY_DISABLED") == "1":
        return
...
def _extrair_licoes(estado: dict, contratos: dict, caminho_agente) -> int:
    ...
    Se MEMORY_DISABLED=1, e no-op (baseline sem memoria nao deve aprender).
    """
    if os.environ.get("MEMORY_DISABLED") == "1":
        return 0
