"""
Carregador de Contratos e Estado — Unidade 4.

Le contratos (.md com YAML) e cria o estado inicial do agente.
Evolucao U4: carrega reflection.md e inicializa memoria a partir do contrato.
O runtime e agnostico — nao sabe qual agente esta rodando.
"""
...
def carregar_contratos(caminho_agente: Path, arquitetura: str = None) -> dict:
    ...
    NOVO U4: carrega reflection.md (backward compatible — arquivo pode nao existir).
    """
    ...
    contratos = {
        ...
        # NOVO U4: reflection.md
        "reflexao": carregar_yaml_do_md(caminho_agente / "reflection.md"),
...
def inicializar_memoria(contratos: dict, caminho_agente: Path) -> tuple:
    ...
    O runtime nao sabe qual agente e — le tudo do contrato.
    Backward compatible: se nao ha tipos_memoria, retorna None.

    Evolucao U4 aula 14: se ``contextual.ativo: true`` no contrato, tambem
    inicializa um EmbeddingAdapter e o expoe como atributo
    ``memory_adapter.embedding_adapter``. Tambem grava ``_caminho_agente``
    dentro de ``config_memoria`` para que o ciclo possa localizar o
    ``reflection_store`` sem precisar propagar o caminho manualmente.
    """
    config_memoria = contratos.get("memoria", {})
    tipos_memoria = config_memoria.get("tipos_memoria")

    # Preservar caminho do agente no proprio config_memoria (truque simples
    # para que _recuperar_contexto e _extrair_licoes encontrem o reflection_store
    # sem alterar assinaturas existentes).
    config_memoria["_caminho_agente"] = str(caminho_agente)

    if not tipos_memoria:
        return None, config_memoria

    # Resolver diretorios relativos ao agente
    ...
    try:
        ...
        memory_adapter = MemoryAdapter(config_memoria)
    except ImportError:
        print("  [memoria] memory_adapter nao encontrado, memoria desativada")
        return None, config_memoria

    # Aula 14: anexar embedding_adapter se memoria contextual ativa
    embedding_adapter = inicializar_embedding(config_memoria)
    if embedding_adapter is not None:
        memory_adapter.embedding_adapter = embedding_adapter
    else:
        memory_adapter.embedding_adapter = None

    return memory_adapter, config_memoria


def inicializar_embedding(config_memoria: dict):
    """Inicializa embedding adapter se memoria contextual esta ativa.

    Retorna EmbeddingAdapter ou None. Configuracao vem do contrato.
    """
    tipos = config_memoria.get("tipos_memoria", {})
    config_contextual = tipos.get("contextual", {})

    if not config_contextual.get("ativo"):
        return None

    try:
        from adapters.embedding_adapter import EmbeddingAdapter
        return EmbeddingAdapter(config_contextual)
    except ImportError:
        print("  [memoria] embedding_adapter nao encontrado, memoria contextual desativada")
        return None
